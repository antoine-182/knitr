...
#' \code{results=tex/lstlisting/hide} are changed to
...
fix_sweave = function(x) {
  ...
  x = gsub_msg('replacing results=lstlisting with results=markup',
               'results\\s*=\\s*lstlisting', 'results=markup', x)
...
which_sweave = function(x) {
  unique(c(
    ...
    grep('^<<.*\\bresults\\s*=\\s*(tex|lstlisting|hide)\\b.*>>=', x),
